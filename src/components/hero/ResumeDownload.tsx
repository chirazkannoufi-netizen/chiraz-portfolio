 ...
 *
 * Files live at `public/cv/chiraz-lina-kannoufi-{locale}.pdf`. Ship all four;
 * if one is missing the visitor gets a 404 rather than a silent no-op, which
 * is the failure mode you actually want to notice.
 */
export function ResumeDownload({
  ...
  const file = `/cv/chiraz-lina-kannoufi-${locale}.pdf`;

  return (
    <a
      href={file}
      download={`Chiraz-Lina-Kannoufi-CV-${locale.toUpperCase()}.pdf`}
      aria-label={aria}
